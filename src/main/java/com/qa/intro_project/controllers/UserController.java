...
public class UserController {
	...
	public User getUser(@PathVariable(name = "id") int id) {
		// we need to get the id out of the path, this can be done with the
		// @PathVariable annotation
		// - the 'name' attribute takes the name of the path variable
		// - this then gets the path variable from the path and inserts it into the
		// parameter 'int id'
...
//		return user;
	...
	public User createUser(@RequestBody User user) {
		// @RequestBody tells Spring to retrieve the data in the requests body and then
		// create an instance
		// of User (using the default empty constructor) and then calls its mutators
		// (setters) to set its values
		user.setId(COUNTER++);
		users.add(user);
		return user;
	}
